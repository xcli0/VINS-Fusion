...
double solveScale(map<double, ImageFrame> &all_image_frame)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        tmp_b = frame_j->second.pre_integration->delta_eta.block<3, 1>(0, 0) - frame_i->second.pre_integration->delta_q * TIO_L + TIO_L
...
void RefineGravity(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    ...
    for(int k = 0; k < 4; k++)
    {
        ...
        for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
        {
            ...
            tmp_b.block<3, 1>(0, 0) = frame_j->second.pre_integration->delta_p - frame_j->second.pre_integration->delta_eta.block<3, 1>(0, 0) + 
                                    frame_j->second.pre_integration->delta_q.toRotationMatrix() * TIO_L - TIO_L -
